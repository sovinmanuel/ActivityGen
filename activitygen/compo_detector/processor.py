...
class ImageProcessor:
    ...
    def read_img(self, path, resize_height=800):
    ...
    def read_and_resize_img(self, path, resize_height=800):
        def resize_by_height(org):
            w_h_ratio = org.shape[1] / org.shape[0]
            resize_w = resize_height * w_h_ratio
            re = cv2.resize(org, (int(resize_w), int(resize_height)))
    ...
    def reverse_binary(self, bin, show=False):
        """
    ...
    def binarization(self, org, grad_min, show=False, write_path=None, wait_key=0):
        grey = cv2.cvtColor(org, cv2.COLOR_BGR2GRAY)
        grad = self.gray_to_gradient(grey)  # get RoI with high gradient
