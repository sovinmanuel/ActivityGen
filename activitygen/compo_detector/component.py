...
def cvt_compos_relative_pos(compos, x1_base, y1_base):
    for compo in compos:
        compo.compo_relative_position(x1_base, y1_base)
...
class Component:
                ...
                if abs(depth) / adj_side > 0.3:
                    ...
                    if abnm / len(border) > 0.1:
                        if test:
                            ...
                            drawer.draw_boundary([self], self.image_shape, show=True)
                        self.rect_ = False
                        return False
                    continue
                else:
    ...
    def compo_relative_position(self, x1_base, y1_base):
        """
        Convert to relative position based on base coordinator
        """
        self.bbox.bbox_cvt_relative_position(x1_base, y1_base)
