...
def merge_text(compos, org_shape, max_word_gad=4, max_word_height=20):
    def is_text_line(compo_a, compo_b):
        (x1_a, y1_a, x2_a, y2_a) = compo_a.put_bbox()
        (x1_b, y1_b, x2_b, y2_b) = compo_b.put_bbox()
        ...
        # on the same line
        # if abs(y1_a - y1_b) < max_word_gad and abs(y2_a - y2_b) < max_word_gad:
        if y1_s < y2_s:
            # close distance
            if (
                x1_s < x2_s
                or (0 < x1_b - x2_a < max_word_gad)
                or (0 < x1_a - x2_b < max_word_gad)
            ):
                return True
        return False
...
def rm_top_or_bottom_corners(
    components,
    org_shape,
    top_bottom_height=parse_int_tuple(
        CONFIG_PARSER.get("MAIN", "threshold_top_bottom_bar")
    ),
):
    new_compos = []
    ...
    for compo in components:
        ...
        # remove big ones
        # if (y2 - y1) / height > 0.65 and (x2 - x1) / width > 0.8:
        #     continue
        if not (
            y2 < height * top_bottom_height[0] or y1 > height * top_bottom_height[1]
        ):
...
def rm_line_v_h(
    binary,
    show=False,
    max_line_thickness=CONFIG_PARSER.getfloat("MAIN", "threshold_line_thickness"),
):
...
def rm_line(
    binary,
    max_line_thickness=CONFIG_PARSER.getfloat("MAIN", "threshold_line_thickness"),
    min_line_length_ratio=CONFIG_PARSER.getfloat("MAIN", "threshold_line_min_length"),
    show=False,
    wait_key=0,
):
...
def rm_noise_in_large_img(
    compos,
    org,
    max_compo_scale=parse_int_tuple(
        CONFIG_PARSER.get("MAIN", "threshold_compo_max_scale")
    ),
):
...
def detect_compos_in_img(
    ...
    for compo in compos:
        if compo.category == "Image":
            ...
            bin_clip = processor.reverse_binary(bin_clip, show=show)

            compos_rec, compos_nonrec = component_detection(
                bin_clip, 25, test=False, step_h=10, step_v=10, rec_detect=True
            )
            for compo_rec in compos_rec:
                compo_rec.compo_relative_position(compo.bbox.x1, compo.bbox.y1)
                if (
            ...
            #     compo_inner.compo_relative_position(compo.bbox.x1, compo.bbox.y1)
...
def compo_block_recognition(
    binary,
    compos,
    block_side_length=CONFIG_PARSER.getfloat("MAIN", "block_side_length"),
):  # TODO: Change block_side_length
...
def component_detection(
    binary,
    min_obj_area,
    line_thickness=CONFIG_PARSER.getfloat("MAIN", "threshold_line_thickness"),
    min_rec_evenness=CONFIG_PARSER.getfloat("MAIN", "threshold_rec_min_evenness"),
    max_dent_ratio=CONFIG_PARSER.getfloat("MAIN", "threshold_rec_max_dent_ratio"),
    step_h=5,
    step_v=2,
    rec_detect=False,
    show=False,
        ...
        for j in range(i % 2, column, step_v):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                if test:
                    ...
                    drawer.draw_boundary([component], binary.shape, show=True)
                ...
                if show:
                    print("Area:%d" % (len(region)))
                    drawer.draw_boundary(compos_all, binary.shape, show=True)
