...
class ImageDrawer:
    ...
    def draw_bounding_box_class(
        self, org, components, line=2, show=False, write_path=None, name="board"
    ):
        """
        Draw bounding box of components with their classes on the original image
        :param org: original image
        :param components: bbox [(x1, y1, x2, y2)]
                            -> top_left: (x1, y1)
                            -> bottom_right: (x2, y2)
    ...
    def draw_line(self, org, lines, color=(0, 255, 0), show=False):
        """
        Draw detected lines on the original image
        :param org: original image
        :param lines: [line_h, line_v]
                -> line_h: horizontal {'head':(x1, row), 'end':(x2, row), 'thickness':int)
                -> line_v: vertical {'head':(column, y1), 'end':(column, y2), 'thickness':int}
    ...
    def draw_boundary(self, components, shape, show=False):
        """
    ...
    def draw_region(self, region, broad, show=False):
    ...
    def draw_region_bin(self, region, broad, show=False):
        for point in region:
